...
void setup() {
  ...
  if (!veml.begin()) {
    Serial.println("VEML 7700 Sensor not found");
    while (1);
  }
...
void loop() {
  ...
  if(time - previousTime >= SAMPLE_PERIOD) {
    ...
    Serial.print("Photo lux : ");
    Serial.println(sensorValueLux);

    Serial.print("VEML lux : ");
    Serial.println(veml.readLux());
    if (veml.interruptStatus() & VEML7700_INTERRUPT_LOW) {
      Serial.println("** Low threshold");
    }
    if (veml.interruptStatus() & VEML7700_INTERRUPT_HIGH) {
